...
class FindBack():
    def __init__(self, graph_dir_path, sp_drimm_dir_path, species):
        '''

        :param graph_dir_path: 图merge合并后，结果文件夹路径
        :param sp_drimm_dir_path: drimmBlocks文件夹路径
        :param species: 物种列表
        '''
        B2M_file_name = 'root-B2M.txt'
        M2B_file_name = 'root-M2B.txt'
        virtual_file_name = 'virtual.txt'
        B2M_path = os.path.join(graph_dir_path, B2M_file_name)
        M2B_path = os.path.join(graph_dir_path, M2B_file_name)
        virtual_path = os.path.join(graph_dir_path, virtual_file_name)

        B2M, M2B = self._read_manual_file(B2M_path, M2B_path)
        sp_block_info, sp_block_signal_info = self._read_block_file(sp_drimm_dir_path, species)
        ...
        # 处理物种原始block
        self._changed_blocks, self.manual_info = self._find_block_no_virtual(sp_block_info, sp_block_signal_info, B2M,
                                                                  block_priority_no_virtual)


        # 2. virtual进行额外处理，统计M的内部block出现的次数来确定方向, 插入virtual, 还原 M
        self._out_blocks = self._find_block_virtual(self._changed_blocks, self.manual_info, virtual_path, B2M)

        # self.get_manual_synteny('./example/drimm-20/synteny.txt', manual_info)

    def _strip_block(self, s):
        return s if not s.startswith('-') else s[1:]

    def _reverse_block_order(self, block: str):
        if block.startswith('-'):
            return block[1:]
        else:
            return '-' + block

    def _merge_ordered_lists(self, list1, list2):
        result = []
        i, j = 0, 0
    ...
    def _read_manual_file(self, B2M_path, M2B_path):
        # 读取上一步生成的所有M信息
    ...
    def _find_block_no_virtual(self, sp_block_info: dict, sp_block_signal_info: dict,
                    ...
                    if last:
                        ...
                        if last != signal_block_row[index]:
                            ...
                            if manual_adj_strand_info:
                                reversed_manual_adj_strand_info = [self._reverse_block_order(i) for i in manual_adj_strand_info[::-1]]
                            ...
                            if manual_strand == '+':
                                ...
                                manual_info[last]['block'].append([self._reverse_block_order(i) for i in original_block_order[::-1]])
                                changed_block_row.append('-' + last)
                            collect = []
                    else:
        ...
        # 为manual_info中block去重
        for m in manual_info.keys():
            block_lists = manual_info[m]['block']
            res = []
                            ...
                            if [unsigned_left_block, unsigned_right_block] == adj \
                                ...
                                if (left_block, right_block) in manual_info[manual_block]['adj']:
                                    ...
                                    manual_info[manual_block]['adj'][
                                        (self._reverse_block_order(right_block),
                                         self._reverse_block_order(left_block))
                                    ] = '-'
    ...
    # 输出结果
    def get_expand_manual_synteny(self, synteny_path):
        def restore_sequence(b: list, b_dict: dict):
            res = []
            for i in b:
                if not i.startswith('-'):
                    res.extend(
        ...
        # 获取源synteny
        original_synteny = {}
        with open(synteny_path,'r') as f:
            for line in f:
                line_info = line.strip().split(':')
                block = line_info[0]
                sequence =  line_info[1].split(' ')[1:]
                original_synteny[block] = sequence

        # 还原manual所属的synteny
        manual_block_info = {}
        for manual in self.manual_info.keys():
            block_lists = self.manual_info[manual]['block']
            sequence = restore_sequence(block_lists[0], original_synteny)
            for i in block_lists[1:]:
                sequence = self._merge_ordered_lists(
                    sequence, restore_sequence(i, original_synteny)
                )
            manual_block_info[manual] = sequence

        original_synteny.update(manual_block_info)
        return original_synteny

    def get_blocks_lists(self):
